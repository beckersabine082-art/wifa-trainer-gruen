...
**Live-Abnahme am 13.09.2026:** Das Dashboard zeigte zunächst null Nutzungen. Anschließend wurden im angemeldeten Trainer ein Glossar-Aufruf und ein Formelsammlungs-Aufruf ausgelöst; nach der Bündelung zeigt das Dashboard genau diese beiden Tageszähler. GA war dabei abgelehnt (sichtbarer Status: Nutzungsanalyse ist aus). Diese zwei Abnahme-Nutzungen verbleiben als dokumentierte Testwerte in den Aggregaten. Nach ausdrücklicher Freigabe des Betreibers wurde ausschließlich usageAdmin kurz entfernt. Derselbe angemeldete Browser erhielt beim erneuten Abruf die serverseitige Zugriffsverweigerung; vorherige Statistikdaten wurden ausgeblendet. Unmittelbar anschließend wurde usageAdmin wiederhergestellt und über Google verifiziert. Der Vergleich bestätigt: alle übrigen Custom Claims unverändert. Der Test hatte zusätzlich eine automatische Wiederherstellung nach spätestens 90 Sekunden als Absicherung. Cloud Shell ist autorisiert; für die gezielte Admin-API-Abfrage war die Quotenprojekt-Zuordnung `x-goog-user-project: wifa-trainer-gruen` erforderlich. Es wurde kein Tarif geändert und keine Functions-Anwendung angelegt.
...
Die Mock-Tests beweisen keine echte Google-Tokenprüfung und kein reales Apps-Script-Deployment. Genau dafür sind die obigen kontrollierten Live-Prüfungen notwendig. Google-Freigabe, private Tabelle, Backend-Deployment und Frontend-Veröffentlichung sind abgeschlossen. Echter Admin-Lesezugriff, Schreiben zweier Testnutzungen und deren erneutes Lesen wurden live bestätigt; ungültige Tokens werden abgewiesen. Auch der ausdrücklich freigegebene Live-Widerrufstest ist erfolgreich abgeschlossen; das Adminrecht ist wiederhergestellt, andere Kontorechte sind unverändert. Der historische Analysebericht bleibt als Ausgangslage erhalten; diese Datei beschreibt die nachfolgende Implementierung.
...
Beim ersten Live-Aufruf enthielt der Browser-Cache noch eine Firebase-Konfiguration ohne den neuen Export. Commit 02279d5 versieht die Statistik-Modulkette mit einer Versionskennung; die anschließende echte Browserprüfung war erfolgreich. Die 256 Tests waren zuletzt nach dieser Korrektur grün und wurden bei der reinen abschließenden Live-Abnahme nicht erneut vollständig ausgeführt.
