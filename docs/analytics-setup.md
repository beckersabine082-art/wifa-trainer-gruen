...
Stand 10.09.2026. Code auf Feature-Branch; **keine Live-Messung bestätigt**. Das vorhandene Firebase-Webprojekt heißt `wifa-trainer-gruen`. Mess-ID fehlte im Repository; der verfügbare Browser zeigte die Google-Anmeldung, keinen autorisierten Projektzugriff. Deshalb wurden keine GA4-Einstellungen, Dimensionen oder Berichte im Konto geändert. `js/analytics-settings.js` bleibt mit `enabled: false` und leerer `measurementId` sicher deaktiviert.

## Aktivierung – gebündelte verbleibende Schritte

1. Im [Firebase-Projekt](https://console.firebase.google.com/project/wifa-trainer-gruen/settings/general) mit dem Betreiberkonto anmelden. Unter Projekteinstellungen → Integrationen die bestehende Analytics-Verknüpfung prüfen, bei Bedarf GA4 im kostenlosen Standardprodukt verbinden. Keine neue Firebase-App, kein Analytics 360, kein BigQuery und kein kostenpflichtiger Zusatzdienst nötig. Wenn Nutzungsbedingungen bestätigt werden müssen, übernimmt das die Betreiberin.
2. Unter Projekteinstellungen → Allgemein → bestehende Web-App `1:561836344573:web:23c2cc88c74a9e8f11aa94` die `measurementId` ermitteln und mit dem zugehörigen GA4-Webdatenstream abgleichen. Diese öffentliche `G-…`-ID in `js/analytics-settings.js` eintragen; Zugangsdaten gehören weder in Code noch Chat. Produktions-Origin und Pfad entsprechen bereits GitHub Pages.
3. In [Google Analytics](https://analytics.google.com/) die verknüpfte Property wählen. Verwaltung → Datenstreams → Webstream → **Optimierte Analysen komplett ausschalten**, einschließlich Seitenänderungen, Formularen, Suche, Downloads, Video und ausgehenden Links. Keine verbundenen zusätzlichen Tags, Google-Ads-Verknüpfungen, User-ID-, User-provided-data- oder Werbekonfiguration ergänzen. Verwaltung → Datenerhebung → **Google Signals ausschalten**; Nutzer-/Werbedatenfreigaben prüfen. Der Code setzt zusätzlich `allow_google_signals: false`, `allow_ad_personalization_signals: false`, verweigert alle Werbe-Consent-Kategorien und setzt `send_page_view: false`.
4. Verwaltung → Datenaufbewahrung: Ereignisdaten auf **2 Monate** stellen, Verlängerung bei neuer Aktivität ausschalten. Die tatsächliche Einstellung und die abweichende Behandlung aggregierter Berichte im Datenschutztext bestätigen. Keine BigQuery-Verknüpfung anlegen.
5. Unter Verwaltung → Benutzerdefinierte Definitionen die nachfolgende Tabelle als **ereignisbezogene** Dimensionen anlegen. Anschließend Berichte/Explorationen gemäß unten vorbereiten. Neu registrierte Dimensionen können 24–48 Stunden benötigen.
6. `docs/analytics-privacy.html`, den identischen Abschnitt in `datenschutz.html` und in `index.html#datenschutzView` rechtlich prüfen und die offen markierten Betreiber-/Google-/Übermittlungs-/Aufbewahrungsangaben vervollständigen. Den Einrichtungsstatus erst nach tatsächlicher Aktivierung ändern.
...
8. Für reale SDK-/Netzwerk-Abnahme eine **separate kostenlose Test-Property/Web-App** nutzen, falls eine vollständig getrennte Testmessung gewünscht ist; die Produktionskonfiguration nicht unbemerkt dafür überschreiben. Alternativ einen bewusst markierten einmaligen Abnahmetest in der Produktions-Property durchführen und diesen als Testdaten dokumentieren. Ohne diese Prüfung keinen Live-Erfolg behaupten. Besonders auch ein direktes Laden einer URL mit synthetischen personenbezogenen Parametern prüfen: diese dürfen nicht in `collect`-Nutzdaten oder Referrer-Headern erscheinen.
9. Nach Konsole/Datenschutz/Netzwerk-Abnahme `enabled: true` setzen, Änderungen prüfen, Feature-Branch nach `main` integrieren und GitHub Pages veröffentlichen. Der aktuelle Auftrag pusht nur den Feature-Branch; das schaltet die bestehende Website nicht automatisch um. Anschließend unter Echtzeit den ersten ausdrücklich freigegebenen realen Eingang prüfen und nach 24–48 Stunden die normalen Berichte kontrollieren.
...
| Podcast-Fortschritt | `progress_percent` | 25, 50, 75, 100 |
...
**Nicht als erledigt melden:** echte Google-Netzwerkprüfung nach Zustimmung, Live-Dashboard-Eingang, Kontoanlage der Dimensionen sowie physische Smartphone-Screen-Lock-Prüfung auf iOS/Android. Bestehende MediaSession-/Visibility-Tests ersetzen keinen physischen Gerätesperrentest.
